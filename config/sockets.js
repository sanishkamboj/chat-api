...
module.exports.sockets = {
  ...
  afterDisconnect: function(session, socket, cb) {
    ...
    async.series(
      [
        function(userCb) {
          User.update({ socketid: socket.id }, reqData).exec(function afterwards(err, user) {
            if (err) {
              cb();
            }
            ...
            if (typeof user != "undefined" && user.length > 0) {
              ...
              sails.log("User went offline", user[0].fullName());
        ...
        function(socketCb) {
          ...
          if (typeof socket.id != "undefined") {
            UserSockets.findOne({ socket_id: socket.id }).exec(function(err, userSocket) {
              console.log("userSocket", userSocket);
              if (typeof userSocket != "undefined") {
                var hours = moment.duration(moment().diff(moment(userSocket.createdAt))).asMinutes();
                UserSockets.update({ socket_id: socket.id }, { time_diff: hours }).exec(function(err, result) {
                  socketCb();
                });
              } else socketCb();
            });
